...
class eVAE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.population_size = getattr(config, 'population_size', 20)  
        self.elite_ratio = getattr(config, 'elite_ratio', 0.3)  
        self.mutation_rate = getattr(config, 'mutation_rate', 0.2)  
        self.crossover_rate = getattr(config, 'crossover_rate', 0.7) 
        self.generations = getattr(config, 'generations', 3) 
        ...
        self.fc_logvar = nn.Linear(self.n_embd, self.latent_dim)
        ...
        self.fitness_net = nn.Sequential(
            nn.Linear(self.latent_dim + self.n_embd, self.n_embd // 2),
            nn.GELU(),
    ...
    def calculate_fitness(self, x_original, z_candidates):
       
        batch_size = x_original.size(0)
        fitness_scores = []
        
        for i in range(self.population_size):
            z = z_candidates[:, i, :]  # [batch_size, latent_dim]
            
           
            x_recon = self.decode(z)
            recon_error = F.mse_loss(x_recon, x_original, reduction='none').mean(dim=-1)  # [batch_size]
            
            
            fitness_input = torch.cat([z, x_original], dim=-1)  # [batch_size, latent_dim + n_embd]
            fitness_raw = self.fitness_net(fitness_input).squeeze(-1)  # [batch_size]
            
            
            fitness = fitness_raw - 0.1 * recon_error
            fitness_scores.append(fitness)
        
        return torch.stack(fitness_scores, dim=1)  # [batch_size, population_size]
    
    def tournament_selection(self, population, fitness_scores, k=3):
        
        batch_size, pop_size, latent_dim = population.shape
        selected = []
        
        for _ in range(2): 
            
            tournament_size = min(k, pop_size)
            tournament_indices = torch.randperm(pop_size)[:tournament_size]
            
            
            tournament_fitness = fitness_scores[:, tournament_indices]  # [batch_size, tournament_size]
            winner_indices = torch.argmax(tournament_fitness, dim=1)  # [batch_size]
            
            
            winner_idx = tournament_indices[winner_indices]  # [batch_size]
            winners = population[torch.arange(batch_size), winner_idx]  # [batch_size, latent_dim]
            selected.append(winners)
        
        return torch.stack(selected, dim=1)  # [batch_size, 2, latent_dim]
    
    def crossover(self, parents):
        
        parent1, parent2 = parents[:, 0], parents[:, 1]  # [batch_size, latent_dim]
        
        # 
        mask = torch.rand_like(parent1) < self.crossover_rate
        offspring = torch.where(mask, parent1, parent2)
        
        return offspring
    
    def mutate(self, individual, mu, logvar):
        
        if torch.rand(1) < self.mutation_rate:
            std = torch.exp(0.5 * logvar)
            noise = torch.randn_like(individual) * std * 0.1
            individual = individual + noise
        return individual
    
    def evolutionary_optimization(self, x, mu, logvar):
        ...
        for _ in range(self.population_size):
            ...
            population.append(z)
        population = torch.stack(population, dim=1)  # [batch_size, population_size, latent_dim]
        
       
        for generation in range(self.generations):
            
            fitness_scores = self.calculate_fitness(x, population)
            
            
            _, elite_indices = torch.topk(fitness_scores, self.elite_size, dim=1)
            elite_population = torch.gather(
                population, 1, 
                elite_indices.unsqueeze(-1).expand(-1, -1, self.latent_dim)
            )
            ...
            for _ in range(remaining_size):
                ...
                offspring = self.mutate(offspring, mu, logvar)
                new_population.append(offspring.unsqueeze(1))
            population = torch.cat(new_population, dim=1)
        
        
        final_fitness = self.calculate_fitness(x, population)
        best_indices = torch.argmax(final_fitness, dim=1)
        best_z = population[torch.arange(batch_size), best_indices]
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = eVAE(config)# CausalSelfAttention  eVAE
    ...
    def configure_optimizers(self, train_config, lr):
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                elif pn in ['residual_weight']:  
                # eVAE residual weight should not be decayed
                    no_decay.add(fpn)
